...
public class Rigid_Bunny : MonoBehaviour 
{
	...
	public List<Transform> collisionPlanes;
	...
	void Collision_Impulse(Vector3 P, Vector3 N)
	{
		...
		for (int i = 0; i < vertices.Length; i++)
		{
			...
			Vector3 xi = x + Rri;
			// xi转到世界坐标系下计算，修复scale不生效问题
			xi = transform.TransformPoint(vertices[i]);
	...
	void Update () 
	{
		...
		foreach (Transform planeTransform in collisionPlanes)
		{
			Vector3 planePosition = planeTransform.position;
			Vector3 planeNormal = planeTransform.up;
			Collision_Impulse(planePosition, planeNormal);
		}
