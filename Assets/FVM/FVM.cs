...
public class FVM : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // 更新Mesh的位置，使其跟随对象的移动
        // Graphics.DrawMesh(mesh, transform.position, transform.rotation, GetComponent<MeshRenderer>().material, 0);
    ...
    void _Update()
    {
    	...
    	for(int i = 0; i < number; i++)
    	{
		    ...
		    if (X[i].y + transform.position.y < FloorYPos) {
			    V[i].y += (FloorYPos - (X[i].y + transform.position.y)) / dt;
			    X[i].y = FloorYPos - transform.position.y;
		    }
	    }
    }
